...
class AppealRegisterMySearch extends AppealRegister
{
    ...
    public function search($params)
    {
        ...
        $query = AppealRegister::find()->where(['company_id'=>$user->company_id])->andFilterWhere(['like','users',"{$user->id}"])
        ->orWhere(['ijrochi_id'=>$user->id])->orWhere(['rahbar_id'=>$user->id])->orderBy(['status'=>SORT_ASC,'deadtime'=>SORT_ASC]);
