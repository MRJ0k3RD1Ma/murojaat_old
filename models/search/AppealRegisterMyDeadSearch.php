...
class AppealRegisterMyDeadSearch extends AppealRegister
{
    ...
    {
        return [
            [['id', 'appeal_id', 'rahbar_id','region_id','district_id','village_id','question_id','gender', 'ijrochi_id', 'parent_bajaruvchi_id', 'deadline', 'status', 'control_id','company_id', 'answer_send', 'nazorat', 'takroriy', 'takroriy_id'], 'integer'],
    ...
    public function search($params)
    {
        ...
        $sql = "(ijrochi_id={$user->id} or rahbar_id={$user->id} or (users like '%\"{$user->id}\"%' and not user_answer like '%\"{$user->id}\"%')) and appeal_register.deadtime<date(now())";
        ...
        ->andWhere(['<>','appeal_register.status',2])->andWhere($sql)->orderBy(['appeal_register.deadtime'=>SORT_ASC]);
